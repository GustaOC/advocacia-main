...
function EntitiesStats({ clients, listType }: { clients: Client[], listType: string }) {
  ...
  const stats = [
    { label: "Total de Clientes", value: totalClientes.toString(), icon: Users, color: "text-brand", bg: "from-brand-light/50 to-brand-light/20", trend: "+12%" },
    { label: "Total de Executados", value: totalExecutados.toString(), icon: Building2, color: "text-brand-sage", bg: "from-brand-sage/30 to-brand-sage/10", trend: "+8%" },
    { label: "Cadastros Ativos", value: clients.length.toString(), icon: User, color: "text-brand", bg: "from-brand-beige/50 to-brand-beige/20", trend: "+15%" },
    { label: "Novos este Mês", value: "23", icon: TrendingUp, color: "text-brand", bg: "from-brand-gray/30 to-brand-gray/10", trend: "+25%" },
  ];
    ...
    <>
      <div className="space-y-6">
        <div className="bg-white border-l-4 border-brand p-8 shadow-sm mb-6 rounded-sm">
          <h2 className="text-3xl font-serif text-brand-black tracking-tight">Gerenciamento de Clientes</h2>
          <p className="text-brand-gray mt-2 font-medium">Acompanhamento e gestão da carteira de clientes e partes relacionadas.</p>
        </div>

...
function AudienciasStats({ events }: { events: CalendarEvent[] }) {
  const stats = [
    { label: "Total de Audiências", value: events.length.toString(), icon: CalendarIcon, color: "text-amber-600", bg: "from-amber-100 to-amber-50", trend: "+2%" },
    { label: "Próximos 7 dias", value: events.filter(e => moment(e.start).isBetween(moment(), moment().add(7, 'days'))).length.toString(), icon: AlertCircle, color: "text-amber-700", bg: "from-amber-200 to-amber-100", trend: "Urgente" },
  ];
...
export function AudienciasModule() {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end mb-8">
        <div className="bg-white border-l-4 border-brand p-8 shadow-sm mb-6 rounded-sm">
          <h2 className="text-3xl font-serif text-brand-black tracking-tight">Agenda de Audiências</h2>
          <p className="text-brand-gray mt-2 font-medium">Acompanhe todas as suas audiências de forma isolada e dedicada.</p>
        </div>
